...
const Clients = ({
  ...
  const visibleCompanies = companies.filter((c) => Boolean(c.enabled));
    ...
    <section id="testimonials" className="py-20">
      ...
      <div className="flex flex-col items-center max-lg:mt-10">
        ...
        {visibleCompanies.length > 0 ? (
          <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-6 max-lg:mt-10">
            {visibleCompanies.map((company) => (
              <React.Fragment key={company.id}>
                <div className="flex items-center gap-2.5">
                  ...
                  <span
                    className="font-[family-name:var(--font-geist-sans)] text-base md:text-xl font-semibold tracking-tight text-white whitespace-nowrap"
                    title={company.name}
                  >
                    {company.name}
                  </span>
