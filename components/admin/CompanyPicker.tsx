  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-slate-900/60 p-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-xs font-medium text-white">
            {current?.label || (chosen ? "Custom path" : "No logo selected")}
          </p>
      ...
      <EditModal
        ...
        title="Pick from company library"
        description="Select an existing company logo to use its path."
        size="xl"
      >
        ...
        {filtered.length === 0 ? (
          <div className="rounded-xl border border-dashed border-white/10 bg-slate-900/30 px-4 py-10 text-center">
            ...
            <p className="mt-2 text-sm text-slate-300">
              {options.length === 0
                ? "No company logos yet."
                : "No logos match your search."}
            </p>
            {options.length === 0 ? (
              <p className="mt-1 text-[11px] text-slate-500">
                Upload a logo from the form above or sync from disk.
              </p>
            ) : null}
          </div>
        ) : (
            ...
            {filtered.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "group relative aspect-square overflow-hidden rounded-lg border bg-slate-900/30 transition",
                    isSelected
