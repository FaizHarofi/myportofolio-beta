...
import { FolderSearch, ImageIcon as ImgIcon, X, Check } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-slate-900/60 p-3">
        <div className="flex h-16 w-24 shrink-0 items-center justify-center overflow-hidden rounded-md border border-white/10 bg-slate-950">
          {current ? (
            <img
              src={current.path}
              alt={current.label}
            ...
            <ImgIcon size={18} className="text-slate-400" />
          )}
        </div>
      ...
      <EditModal
        ...
        description="Pick from uploaded assets or paste a path."
        size="xl"
      >
          ...
          <div className="rounded-xl border border-dashed border-white/10 bg-slate-900/30 px-4 py-10 text-center">
            ...
            {options.length === 0 ? (
              <p className="mt-1 text-[11px] text-slate-500">
                Upload a cover from the Assets page.
              </p>
            ) : null}
          </div>
        ) : (
